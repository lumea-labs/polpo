...
export function loopRuntimeErrorEnvelope(
  err: unknown,
): { message: string; type: "loop_runtime_error"; code: "tool_policy_denied" | "loop_policy_blocked" | "loop_permission_blocked" | "loop_approval_required" | "loop_hook_failed" | "loop_binding_invalid" | "loop_binding_missing" | "loop_context_readonly" | "loop_tool_input_invalid" | "loop_agent_input_invalid" | "loop_agent_input_too_large" | "loop_result_invalid"; approvalRequestId?: string; loopRunId?: string } | null {
  ...
  if (err instanceof LoopAgentInputError) {
    return {
      message,
      type: "loop_runtime_error",
      code: err.code,
  ...
  if (err instanceof LoopResultProjectionError) {
    return {
      message,
      type: "loop_runtime_error",
      code: err.code,
