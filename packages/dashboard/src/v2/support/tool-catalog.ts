 ...
 * `requiresKey` → needs an external credential (vault entry, env var, or managed Connection) to run.
 */
...
export interface CatalogGroup {
  ...
  requiresKey?: boolean;
  /** Display text for the credential requirement badge. Defaults to "needs vault". */
  credentialLabel?: string;
  /** Keep every tool visible in allowedTools instead of collapsing to a wildcard. */
  explicitToolsOnly?: boolean;
...
export const TOOL_CATALOG: CatalogGroup[] = [
  ...
  },
  {
    ...
    label: "Browser",
    icon: "Globe",
    note: "Browser automation via the agent-browser runtime. Set a browser profile on the agent to persist cookies and localStorage across runs.",
    tools: [
  ...
  },
  {
    ...
    label: "Image",
    icon: "Image",
    note: "Image generation + vision analysis — runs through the managed gateway, no key needed.",
    tools: [
  ...
  },
  {
    ...
    label: "Video",
    icon: "Video",
    note: "Video generation — runs through the managed gateway, no key needed.",
    tools: [
  ...
  },
  {
    ...
    label: "Audio",
    icon: "Volume2",
    note: "Speech-to-text + text-to-speech — runs through the managed gateway (Edge TTS is free).",
    tools: [
  ...
  },
  {
    ...
    icon: "GithubLogo",
    requiresKey: true,
    credentialLabel: "needs connection",
    explicitToolsOnly: true,
    note: "Uses a project Connection. OAuth token is resolved server-side and never shown to the model.",
    tools: [
      { name: "github_list_repos", description: "List repositories visible to the connected GitHub account." },
      { name: "github_create_issue", description: "Create an issue in an authorized repository." },
      { name: "github_read_file", description: "Read a repository file through a connected GitHub account." },
    ],
  },
  {
    key: "slack_",
    ...
    credentialLabel: "needs connection",
    explicitToolsOnly: true,
    note: "Uses a project Connection. Slack tokens are resolved server-side and never shown to the model.",
    tools: [
      { name: "slack_list_channels", description: "List channels visible to the connected Slack workspace." },
      { name: "slack_send_message", description: "Send a message to a channel in the connected workspace." },
    ],
  },
  {
    key: "drive_",
    ...
    credentialLabel: "needs connection",
    explicitToolsOnly: true,
    note: "Uses a project Connection. Drive tokens are resolved server-side and never shown to the model.",
    tools: [
      { name: "drive_search_files", description: "Search files in the connected Google Drive account." },
      { name: "drive_read_file", description: "Read a Drive file. Google Docs are exported as text/plain." },
    ],
  },
];
...
export const CATALOG_TOOL_WILDCARDS = new Set(
  TOOL_CATALOG.filter((g) => g.key.endsWith("_") && !g.explicitToolsOnly).map((g) => `${g.key}*`),
);
...
/** Tools the runtime grants when an agent has no explicit allowedTools. */
export const BASELINE_TOOLS = new Set([
  "read",
  "write",
  "edit",
  "bash",
...
/**
 * The number of tools an agent actually has enabled — resolving core tools
 * (always on), the empty-allowedTools baseline, and `prefix*` wildcards. Use
 * this instead of `allowedTools.length`, which miscounts wildcards/baseline.
 */
export function countEnabledTools(allowed: string[]): number {
  let n = 0;
  for (const group of TOOL_CATALOG) {
    for (const tool of group.tools) {
      const on = group.core
        ? true
        : allowed.length === 0
          ? BASELINE_TOOLS.has(tool.name)
          : isToolEnabled(tool.name, allowed);
      if (on) n += 1;
    }
  }
  return n;
